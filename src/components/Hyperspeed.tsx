...
type ShaderCallback = Parameters<NonNullable<THREE.ShaderMaterial["onBeforeCompile"]>>[0];
...
const defaultOptions: HyperspeedOptions = {
  ...
  carLightsLength: [420 * 0.03, 420 * 0.22],
...
const distortions: Record<string, Distortion> = {
  ...
  deepDistortion: {
    ...
    getJS: (progress: number, time: number) => {
      ...
      const getY = (p: number) => Math.pow(Math.abs(p * uPowY.x), uPowY.y) + Math.sin(p * Math.PI * uFreq.y + time) * uAmp.y;
...
class CarLights {
  ...
  init() {
    ...
    material.onBeforeCompile = (shader: ShaderCallback) => {
...
const Hyperspeed: FC<HyperspeedProps> = ({ effectOptions = {} }) => {
  ...
  return <div id="lights" className="h-full w-full" ref={hyperspeed} />;
};
